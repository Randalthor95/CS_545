...
try:
    ...
    if result[:4] == correct[:4] and np.allclose(result[5:], correct[5:], atol=5):
        exec_grade += pts
...
try:
    ...
    if first_5 and np.allclose(results_means, correct_means, atol=5):
        exec_grade += pts
        print(f'\n--- {pts}/{pts} points. train_this_partition correctly returns')
        for res in results:
            print(res)
...
try:
    ...
    if np.allclose(means, correct, atol=5):
        exec_grade += pts
